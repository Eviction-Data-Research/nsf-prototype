...
from ..utils.consts import REQUIRED_COLS, MAX_BATCH_SIZE, GEOCODING_API_URL, PROXIMITY_RADIUS
...
relevant_tags = set(['AddressNumber', 'AddressNumberPrefix', 'AddressNumberSuffix', 'StreetName', 'StreetNamePreDirectional', 'StreetNamePreModifier', 'StreetNamePreType', 'StreetNamePostDirectional', 'StreetNamePostModifier', 'StreetNamePostType'])
...
def transform_eviction_data(df: pd.DataFrame, col_mapper: dict):
    ...
    deduplicated_df_copy['standardizedAddress'] = deduplicated_df.apply(lambda row: _evictions_address_concat_standardize(row['defendantAddress1'], row['defendantCity1']), axis = 1)
...
def _get_exact_address_matches(db: Session):
    ...
    select_query = """SELECT * FROM "new-evictions" AS e WHERE e."caresId" IS NOT NULL"""
...
async def _perform_geocode_request(segment: pd.DataFrame, session):
    ...
    async with session.post(url=GEOCODING_API_URL, data=data) as response:
        ...
        output_df = pd.read_csv(StringIO(raw_resp),
                                names=['caseID', 'address', 'match', '_', 'inputAddress', 'location', 'tigerId', 'tigerIdSide'], 
...
async def _geocode_unmatched_eviction_data(unmatched_df: pd.DataFrame):
    ...
    batch_geocode_input_df = batch_geocode_input_df.apply(_parse_city_zip, axis = 1)[['caseID', 'standardizedAddress', 'city', 'state', 'zip']]

    segments = batch_geocode_input_df.groupby(batch_geocode_input_df.index // MAX_BATCH_SIZE)

    async with aiohttp.ClientSession() as session:
        batch_geocode_outputs = await asyncio.gather(*(_perform_geocode_request(segment, session) for _, segment in segments))
...
async def _get_proximity_matches(db: Session):
    query = db.query(TempEviction).filter(not_(exists().where(TempEviction.__table__.c.standardizedAddress == Cares.standardizedAddress)))
    ...
    update_query = f"""
      ...
      SET "closestCaresDistance" = (
          ...
          FROM cares 
          WHERE "new-evictions".location IS NOT NULL AND
                "new-evictions"."caresId" IS NULL
          ORDER BY "new-evictions".location <-> cares.location 
          LIMIT 1
      )
      WHERE "new-evictions".location IS NOT NULL AND "new-evictions"."caresId" IS NULL
      """
